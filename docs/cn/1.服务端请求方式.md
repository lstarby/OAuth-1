...
3. 签名计算方式
    ...
    5. `CanonicalizedOnesHeaders`：**此部分用于让用户添加自定义的签名HEADER，如果没有此需求，直接传入空字符串`""`即可**
        ...
        - 在拼接结果最后再添加一个换行符`"\n"`
    6. `Path`： 如：`https://ones.game/account/register`中即为`/account/register`
    7. 用来签名的字符串为UTF-8格式
...
5. 签名计算示例
    ...
    2. 请求示例： 
        1. curl
            ...
            ```
            curl -H "auth: ThisIsAccessKey:uoKejDFZAfA27eZHbzSsBYoPk4Y=" -H "Content-Type: application/json" -H "Content-Sha1: 123abc" -H "date: Mon, 01 Jan 2018 08:08:08 GMT" -H "Dragonex-Atruth: OnesIsTheBest" -H "ones-btruth: OnesIsTheBest2" -d "" "https://openapi.ones.io/api/v1/token/new/"
            ```
        2. `HTTP_METHOD`：POST
